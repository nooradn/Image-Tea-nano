...
class BatchWorker(QThread):
    def __init__(self, api_key, model, batch, service, metadata_func, row_map, parent=None):
    ...
    def run(self):
        ...
        with ThreadPoolExecutor(max_workers=len(self.batch)) as executor:
            for row in self.batch:
                ...
                image_path = row[1]
                prompt = None
                futures.append(executor.submit(self.metadata_func, self.api_key, self.model, image_path, prompt))
            for idx, future in enumerate(as_completed(futures)):
                if self._should_stop:
                    break
                try:
                    result = future.result()
                    cache_results.append((idx, result))
                except Exception as e:
                    errors.append(str(e))
                self.signals.progress.emit(idx + 1, len(self.batch))
...
def _run_next_batch(window):
    ...
    def on_finished(errors):
        if state.get('should_stop', False):
            # Update all "stopping" rows in this batch to "stopped" in DB and UI
            for row in batch:
                filepath = row[1]
